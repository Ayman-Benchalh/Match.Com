...
<div class="contentOnprodcut">
    ...
    <div class="parttwo">
          ...
          {{-- <form action="{{ route('commandCollection',['idUser'=>$idUser , 'idProduct'=>$idProduct])}}" method="post"> --}}
            <form action="/session/{{ $dataOneprodt->idProduct }}/{{ $dataOneUser->idUser }}" method="POST">
            @csrf
            <input type="hidden" name="total" value="{{$dataOneprodt->prix}}">
            <input type="hidden" name="productname" value="{{$dataOneprodt->nom}}">
